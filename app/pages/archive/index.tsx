...
const ArchivesView: React.FC = () => {
  ...
  const [archivesQuery] = useQuery(getAllSpacesWithAll, {})

  const rawData = [...archivesQuery.spaces, ...archivesQuery.topics, ...archivesQuery.sections]
  console.log("archivesQuery", rawData)
  ...
  return (
    <Grid container padding={2} marginTop={"64px"} paddingX={8} xs={12}>
      ...
      {archivesQuery.sections.length > 0 ||
      archivesQuery.topics.length > 0 ||
      archivesQuery.spaces.length > 0 ? (
        ...
        <Grid container display="flex" flexDirection="column" item xs={12} paddingY={4}>
          ...
          <Grid item>
            <Typography fontSize={"16px"} fontWeight={400}>
              You can send files you don’t need to the archive. Archived files are kept here until
              you restore them or delete them forever.
            </Typography>
          </Grid>
        </Grid>
      )}
...
ArchivesPage.getLayout = (page) => <Layout>{page}</Layout>
